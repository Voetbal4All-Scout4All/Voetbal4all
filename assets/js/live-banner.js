// V4A_LEAGUE_FLAGS_V1
const V4A_LEAGUE_META = {
  JPL:  { name: "Jupiler Pro League", flag: "assets/img/sources/flag-be.svg" },
  ERED: { name: "Eredivisie",        flag: "assets/img/sources/flag-nl.svg" },
  EPL:  { name: "Premier League",    flag: "assets/img/sources/flag-gb.svg" },
  BUND: { name: "Bundesliga",        flag: "assets/img/sources/flag-de.svg" },
  LIGA: { name: "La Liga",           flag: "assets/img/sources/flag-es.svg" },
  SA:   { name: "Serie A",           flag: "assets/img/sources/flag-it.svg" },
  L1:   { name: "Ligue 1",           flag: "assets/img/sources/flag-fr.svg" },
  INT:  { name: "Internationaal",    flag: "assets/img/sources/flag-int.svg" },
};

function v4aFlagForLeague(codeOrName) {
  const raw = String(codeOrName || "").trim();
  const key = raw.toUpperCase();
  if (V4A_LEAGUE_META[key]) return V4A_LEAGUE_META[key];
  // Try to map by known names
  const low = raw.toLowerCase();
  if (low.includes("jupiler")) return V4A_LEAGUE_META.JPL;
  if (low.includes("eredivisie")) return V4A_LEAGUE_META.ERED;
  if (low.includes("premier")) return V4A_LEAGUE_META.EPL;
  if (low.includes("bundesliga")) return V4A_LEAGUE_META.BUND;
  if (low.includes("la liga")) return V4A_LEAGUE_META.LIGA;
  if (low.startswith("serie a") or "serie a" in low): return V4A_LEAGUE_META.SA
  if (low.includes("ligue 1") or low.includes("ligue1")) return V4A_LEAGUE_META.L1;
  return V4A_LEAGUE_META.INT;
}

function v4aLeagueBadge(codeOrName) {
  const m = v4aFlagForLeague(codeOrName);
  const label = m && m.name ? m.name : String(codeOrName || "Internationaal");
  const flag = m && m.flag ? m.flag : "assets/img/sources/flag-int.svg";
  return `<span class="v4a-league-badge" style="display:inline-flex;align-items:center;gap:6px;">
    <img src="${flag}" alt="" width="18" height="12" loading="lazy" decoding="async" style="border-radius:2px;opacity:.95"/>
    <span>${label}</span>
  </span>`;
}
