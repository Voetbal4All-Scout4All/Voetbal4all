...
const ProtectedRoute = ({ children }) => {
  ...
  if (loading) {
    return (
      <div className="s4a-root" style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ width: 40, height: 40, border: '3px solid var(--s4a-border)', borderTopColor: 'var(--s4a-blue)', borderRadius: '50%', animation: 'spin 0.6s linear infinite' }} />
      </div>
    );
  }
    ...
    <BrowserRouter basename="/scout">
      <AuthProvider>
        <Routes>
          ...
          <Route path="/" element={<ProtectedRoute><ScoutLayout /></ProtectedRoute>}>
...
const Placeholder = ({ titleKey }) => {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '60vh', flexDirection: 'column', gap: 'var(--s4a-space-4)' }}>
      <h2 style={{ fontFamily: 'var(--s4a-font-display)', fontSize: '1.3rem', textTransform: 'uppercase', letterSpacing: '0.04em', color: 'var(--s4a-text-muted)' }}>
        {t(titleKey)}
      </h2>
